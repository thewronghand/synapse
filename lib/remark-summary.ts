/**
 * remark 플러그인: :::summary 블록을 커스텀 HTML wrapper로 변환
 *
 * 마크다운에서 fenced block 스타일로 요약을 삽입할 수 있게 한다.
 * 예:
 *   :::summary 요약
 *   회의에서 논의한 핵심 내용입니다.
 *   :::
 *
 * 변환 결과: <div data-summary="요약">마크다운 내용</div>
...
export default function remarkSummary() {
  return (tree: Root) => {
    ...
    while (i < children.length) {
      ...
      if (node.type === "paragraph" && node.children.length >= 1) {
        ...
        const startMatch = firstLine.match(SUMMARY_START);
        ...
        const label = isOpen ? (rest.slice(4).trim() || "요약") : (rest || "요약");
        ...
        if (endIndexInSameNode !== -1) {
          ...
          children.splice(i, 1,
            { type: "html", value: `<div data-summary="${label.replace(/"/g, "&quot;")}"${openAttr}>` } as unknown as RootContent,
        ...
        for (let j = i + 1; j < children.length; j++) {
          ...
          if (candidate.type === "paragraph" && candidate.children.length >= 1) {
            ...
            if (candidateChild.type === "text") {
              ...
              if (SUMMARY_END.test(candidateLines[0].trim())) {
                endIndex = j;
                // ::: 뒤에 추가 텍스트가 있으면 보존
        ...
        const openTag = { type: "html", value: `<div data-summary="${label.replace(/"/g, "&quot;")}"${openAttr}>` } as unknown as RootContent;
